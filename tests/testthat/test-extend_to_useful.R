test_that("C matrices are created correctly", {
  ...
  tryCatch({
    ...
    WithCmats_target <- readd(SEAPSUTWorkflow::target_names$Cmats, character_only = TRUE, path = testing_setup$cache_path)
...
test_that("eta_fu and phi_u vectors are created correctly", {
  ...
  tryCatch({
    ...
    WithEtaPhi_target <- readd(SEAPSUTWorkflow::target_names$EtaPhivecs, character_only = TRUE, path = testing_setup$cache_path)
